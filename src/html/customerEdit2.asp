...
<form name="userEditForm" action="<%= CONTROLLER %>" method="post">
  ...
  <table border="0" cellpadding="0" cellspacing="0" style="float:left;width:350px;">
    ...
    <tr>
      <td class="required" colspan="2"><h2>Email Updates</h2>
        From time to time Yamaha Music Australia might send out an email updating customers about Yamaha Music Australia news &amp events and the Yamaha product range. Do you wish to receive these notices? </td>
    </tr>
    <tr>
      <td class="required" colspan="2"><input type="checkbox" name="optinnews" style="float:left;margin:0 5px 20px 0;" value="1" <%= C._getCustomerOptinnews()==1?" checked=\"checked\"":"" %> />
        Check this to be sent News and Event information about Yamaha Music Australia.</td>
    </tr>
    <tr>
      <td class="required" colspan="2"><input type="checkbox" name="optinproduct" style="float:left;margin:0 5px 20px 0;" value="1" <%= C._getCustomerOptinproduct()==1?" checked=\"checked\"":"" %> />
        Check this to be notified of new Yamaha Product releases.</td>
    </tr>
    <tr>
      <td class="required" colspan="2"><input type="checkbox" name="optinteacher" style="float:left;margin:0 5px 20px 0;" value="1" <%= C._getCustomerOptinteacher()==1?" checked=\"checked\"":"" %> />
        Check this to receive information regarding Yamaha band &amp; orchestral workshops, professional development sessions and band director workshops.</td>
    </tr>
    <tr>
      <td class="required">Image Verification</td>
      <td class="required"><img src="image_ver.asp" alt="Image Verification" width="110" height="35" name="ver_code" /><br />
        <i>Can't see it? <a href="#" onclick="reloadImage()">Reload</a> for a another code</i></td>
    </tr>
    <tr>
      <td class="required"><strong>Retype it:</strong></td>
      <td class="required"><input name="verCode" type="text" id="verCode" /></td>
    </tr>
  </table>
    ...
    <%
		...
		if (rsAllProduct && !rsAllProduct.EOF) {
			while (!rsAllProduct.EOF) {
				...
				%>
    <input type="checkbox" name="productid" value="<%= rsAllProduct("id") %>"<%= blnCatMatch?" checked=\"checked\"":"" %>>
    <%= rsAllProduct("name") %><br/>
    <%
...
%>
  ...
  <p>
    <input type="submit" name="submit" value="<%= C._getCustomerID()==0?"Save your details":"Update your details" %>" class="button" />
  </p>
</form>
