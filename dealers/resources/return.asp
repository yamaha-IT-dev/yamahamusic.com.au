...
function searchItem(){
    ...
    document.location.href = 'return.asp?type=search&txtSearch=' + strSearch + '&status=' + strStatus;	
}
   
function resetSearch(){
	document.location.href = 'return.asp?type=reset';
}
</script>
</head>
<body>
...
sub displayGRA
	...
	if not DB_RecSetIsEmpty(rs) Then
		...
		For intRecord = 1 To rs.PageSize
			...
			'if rs("job_status") = 1 then
			'	strDisplayList = strDisplayList & "<a href=""submit_job.asp?id=" & rs("gra_id") & """>Submit"
			'else
				'strDisplayList = strDisplayList & "<a href=""edit_return.asp?job_id=" & rs("gra_id") & """>Edit"
			'end if
        ...
        strDisplayList = "<tr><td colspan=""13"" >No Goods Return found.</td></tr>"
	end if
	
	strDisplayList = strDisplayList & "<tr>"
	strDisplayList = strDisplayList & "<td colspan=""13"" >"
	strDisplayList = strDisplayList & "<form name=""MovePage"" action=""return.asp"" method=""post"">"
...
<table align="center" cellpadding="0" cellspacing="0" class="main_table">
  <tr>
    <td class="login_column"><h1>Goods Return</h1>
    <p>Our online Goods Return process has been implemented to speed up your request and provide you with a better experience from Yamaha. Please click the below link. This will be processed by our Internal Sales Support.</p>
      <p><a href="new-return.asp">
      <button type="button" class="btn btn-primary btn-lg">New Return</button>
      </a></p>
      <div class="alert alert-search">
        <form name="frmSearch" id="frmSearch" action="return.asp?type=search" method="post" onsubmit="searchItem()">
          <input type="text" name="txtSearch" size="40" value="<%= request("txtSearch") %>" maxlength="20" placeholder="Search Model no / Serial no / Invoice no" />
  ...
  <tr>
    <td><table cellspacing="0" cellpadding="5" class="database_records" width="100%">
        <tr bgcolor="#CCCCCC"> 
      ...
      <ol>
        ...
        <li>Once the return has been approved, you will receive an email from <a href="mailto:mpd_returns@gmx.yamaha.com">mpd_returns@gmx.yamaha.com</a> with a GRC (Goods Return Code). When enquiring about your goods return, please quote this number.</li>
