...
Function getUser(intID)
    ...
    if not DB_RecSetIsEmpty(rs) Then
        ...
        session("user_email")               = rs("username")
        session("user_storename")           = rs("dealer")
        session("user_dealercode")          = rs("dealercode")
        session("user_datecreated")         = rs("datecreated")
        session("user_datemodified")        = rs("datemodified")
        session("user_truncate_dealercode") = rs("truncate_dealercode")
        session("user_focus")               = rs("focus")
    end if
...
' UPDATE USER
'-----------------------------------------------
Function updateUser(intID, strFirstname, strLastname, strAddress, strCity, strState, intPostcode, strPhone, strStorename)
    dim strSQL
    ...
    strSQL = strSQL & "dealer = '" & Server.HTMLEncode(strStorename)& "',"
